...
class Database:
    ...
    def __init__(self,
                 uri=settings.N4D_DATABASE_URI,
                 id=settings.N4D_DATABASE_ID,
                 password=settings.N4D_DATABASE_PASSWORD,
                 encrypted=settings.N4D_DATABASE_ENCRYPTED,
                 trust=settings.N4D_DATABASE_TRUST,
                 max_connection_lifetime=settings.N4D_DATABASE_MAX_CONNECTION_LIFETIME,
                 max_connection_pool_size=settings.N4D_DATABASE_MAX_CONNECTION_POOL_SIZE,
                 connection_acquisition_timeout=settings.N4D_DATABASE_CONNECTION_ACQUISITION,
                 connection_timeout=settings.N4D_DATABASE_CONNECTION_TIMEOUT,
                 max_retry_time=settings.N4D_DATABASE_MAX_RETRY_TIME):
        self.uri = uri
        self.id = id
        ...
        self.initial_connection_attempts_number = settings.N4D_INITIAL_CONNECTION_ATTEMPTS_NUMBER
        self.driver = None
...
class Session:
    ...
    def check_and_set_session_type(session_type):
        if session_type not in ['WRITE', 'READ', None]:
            raise N4DSessionError("The current session's type must be defined on 'WRITE', 'READ' or None.")
        else:
            return session_type
...
class Transaction:
    ...
    def __enter__(self):
        if self.active_transaction is None:
            ...
            raise N4DTransactionConflictError('A transaction is already running...')
